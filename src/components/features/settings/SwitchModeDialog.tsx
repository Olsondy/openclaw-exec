...
import { TriangleAlert, Download, CheckCircle, Loader2 } from 'lucide-react'
...
export function SwitchModeDialog({ fromMode, toMode, onConfirm, onCancel }: Props) {
  const { licenseKey, licenseId, expiryDate, runtimeConfig, approvalRules, resetForModeSwitch } =
    useConfigStore()
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="bg-card-bg border border-card-border rounded-xl shadow-2xl ring-1 ring-white/10 w-full max-w-sm mx-4 p-5 space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-surface-on">切换连接方式</h3>
        </div>
        ...
        <ul className="text-xs text-surface-on-variant space-y-1 pl-1">
          {['当前连接 Token / Gateway', 'License Key（或本地配对会话）', '审批规则将重置为默认值'].map(
            (item) => (
        ...
        <div className="flex gap-2 pt-1">
          <Button variant="outlined" className="flex-1" onClick={onCancel} disabled={confirming}>
            取消
          </Button>
          <Button className="flex-1" onClick={handleConfirm} disabled={confirming}>
            {confirming ? <Loader2 size={14} className="animate-spin" /> : '确认切换'}
          </Button>
        </div>
      </div>
    </div>
